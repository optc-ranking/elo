...
function gotData(snapshot){
	...
	for(x = max - 1; x >= 0; x--){
		var temp = g_TMs[x];rankImg[x] = new Image();
	...
	for(x = max - 1; x >= 0; x--){
		...
		btn3.innerHTML = temp.g_elo.toFixed(1);
		btn2.id = wrapper[x].id;
		btn2.addEventListener("click", function(){window.open(this.id)}, false);
...
function generatePair(){
	...
	a_id = g_TMs[a].unit_id;
	b_id = g_TMs[b].unit_id;
	...
	left.innerHTML = "Vote" + "<br />" + g_TMs[a].name;
	right.innerHTML = "Vote" + "<br />" + g_TMs[b].name;
}
...
left.onclick = function(){	
	...
	var authS = firebase.auth().onAuthStateChanged(function(user) {
		if (user) {
			database.ref("TMs").orderByChild("unit_id").on("value", gotData2, errData);
			TMs.sort(function(a, b){return a.unit_id - b.unit_id});
			
			var winner = find(TMs, a_id);
			var loser = find(TMs, b_id);
			
			var adj = match(4,TMs[winner],TMs[loser], false);
			
			var updates = {};
			updates[winner + "/g_elo"] = TMs[winner].g_elo + adj;
			updates[loser + "/g_elo"] = TMs[loser].g_elo - adj;
...
right.onclick = function(){
	var authS = firebase.auth().onAuthStateChanged(function(user) {
		if (user) {
			...
			var winner = find(TMs, b_id);
			var loser = find(TMs, a_id);
