...
function gotData(snapshot){
	...
	g_raids = filter(raids);
	max = g_raids.length;
	
	generatePair();
		
	for(x = max - 1; x >= 0; x--){
		var temp = g_raids[x];rankImg[x] = new Image();
...
function generatePair(){
	...
	a_id = g_raids[a].unit_id;
	b_id = g_raids[b].unit_id;
	...
	left.innerHTML = "Vote" + "<br />" + g_raids[a].name;
	right.innerHTML = "Vote" + "<br />" + g_raids[b].name;
}
...
left.onclick = function(){	
	...
	var authS = firebase.auth().onAuthStateChanged(function(user) {
		if (user) {
			...
			updates[loser + "/g_elo"] = raids[loser].g_elo - adj;
