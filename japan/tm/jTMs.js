...
function gotData(snapshot){
	...
	for(x = max - 1; x >= 0; x--){
		var temp = j_TMs[x];rankImg[x] = new Image();
...
function generatePair(){
	...
	a_id = j_TMs[a].unit_id;
	b_id = j_TMs[b].unit_id;
	...
	left.innerHTML = "Vote" + "<br />" + j_TMs[a].name;
	right.innerHTML = "Vote" + "<br />" + j_TMs[b].name;
}
...
left.onclick = function(){	
	...
	var authS = firebase.auth().onAuthStateChanged(function(user) {
		if (user) {
			...
			var adj = match(5,TMs[winner],TMs[loser], false);
			
			var updates = {};
			updates[winner + "/j_elo"] = TMs[winner].j_elo + adj;
			updates[loser + "/j_elo"] = TMs[loser].j_elo - adj;
